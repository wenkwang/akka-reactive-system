...
object Async {
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation1().flatMap {
      futureA => makeAsyncComputation2().map(futureB => (futureA, futureB))
    }
  }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    if (maxAttempts < 1) Future.failed(new IllegalArgumentException)
    else makeAsyncComputation().recoverWith {
